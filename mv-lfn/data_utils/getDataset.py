...
class GetDataTrain(Dataset):
    ...
    def __init__(self, dataType='train', imageMode='RGB',views=12):

        self.dt = dataType
        self.imd = imageMode
        self.views = views 
        path = '/home/dh/zdd/data/modelnet_view12'
        ...
        for c,cls in enumerate(clses):
            ...
            for obj in objes:
                ...
                views_path = np.array([os.path.join(obj_path, v) for v in sorted(os.listdir(obj_path))])
                self.fls.append(views_path)
                self.las.append(c)
                self.names.append(obj_path)
