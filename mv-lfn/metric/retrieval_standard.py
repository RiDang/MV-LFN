...
index = 7
name = 'shapenet'
path = ['model_feat.npy','model_feat.npy' ]
...
def interpolation(data):
    ...
    archor=0
    for i in range(20):
        for index,rs in enumerate(data[0][archor:]):
            index = index + archor 
            if rs>i*0.05:
                y[i] = ((data[1][index]-data[1][index-1])/(data[0][index]-data[0][index-1])) * (i*0.05-data[0][index-1]) + data[1][index-1]
                anchor = index
                break
    return y
...
def get_pr(dist_p=0):
    ...
    for i in tqdm(range(len(np.unique(las_qs)))):
        if (las_qs==i).sum()==0: continue
        fts_q = fts_qs[las_qs==i]
        las_q = np.array([i]).repeat(len(fts_q))
        data = [fts_q,las_q,fts_c,las_c]
        evaludation, pr = get_pr_cls(dist_p, data)
        prs.append(interpolation(pr))
        results.append(evaludation)
    #for i in mAP: print('%.3f',end=',') 
    #print('')
    np.savetxt('pr_standard.txt', [(np.arange(20)+1)/20, np.array(prs).mean(axis=0)])
